...
module.exports.getPlatform = str => {
  ...
  if (strWithPlatformMaybe) {
    if (strWithPlatformMaybe.includes('oculus') || strWithPlatformMaybe.includes('go')) {
      return PLATFORMS.oculusvr;
    }
    if (strWithPlatformMaybe.includes('daydream') || strWithPlatformMaybe.includes('pixel')) {
      return PLATFORMS.daydream;
    }
    if (strWithPlatformMaybe.includes('wave') || strWithPlatformMaybe.includes('htc')) {
...
module.exports.parseLicenseFromPkg = pkgJson => {
  let licenseType = null;
  let licenseUrl = pkgJson.homepage + '#license';
  if (pkgJson.license === 'string') {
    licenseType = pkgJson.license;
  } else if (typeof pkgJson.license === 'object') {
    if (pkgJson.license.type) {
      licenseType = pkgJson.type;
    }
    if (pkgJson.license.url) {
      licenseUrl = pkgJson.url;
    }
  }
  return {licenseType, licenseUrl};
};

module.exports.parseBugsFromPkg = pkgJson => {
  let bugsUrl = typeof pkgJson.bugs === 'string' ? pkgJson.bugs : '';
  let bugsSubmitUrl;
  if (typeof pkgJson.bugs === 'object') {
    if (pkgJson.bugs.url) {
      bugsUrl = pkgJson.bugs.url;
    }
    if (pkgJson.bugs.submit_url) {
      bugsSubmitUrl = pkgJson.bugs.submit_url;
    }
  }
  const bugsListUrl = (bugsUrl || '').replace(/\/new/i, '').replace(/\/+$/g, '');
  if (bugsListUrl.includes('github.com') && !bugsSubmitUrl) {
    bugsSubmitUrl = `${bugsListUrl}/new`;
  }
  return {bugsListUrl, bugsSubmitUrl};
};

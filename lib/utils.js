...
module.exports.ngrok = async (
  port = SETTINGS.ngrok.port,
  subdomain = SETTINGS.ngrok.subdomain,
  authtoken = SETTINGS.ngrok.authtoken
) => {
  if (!SETTINGS.ngrok || !SETTINGS.ngrok.enabled) {
    return Promise.reject(new Error('ngrok is disabled'));
  }
  const ngrok = require('ngrok');
  console.log('ngrok…', port, subdomain, authtoken);
  return ngrok.connect({addr: port, subdomain, authtoken})
    .then(url => {
      console.log('connecting to ngrok…', url);
      return url;
    }).catch(err => {
      if (err.code === 'ECONNREFUSED') {
        throw new Error(`Connection refused at ${err.address}:${err.port}`);
      }
      throw err;
    });
};

...
function parseOptions (action, url, defaults = {}) {
  let optionDefinitions = [
    {name: 'platform', alias: 'p', type: String, multiple: true, defaultValue: [
      action === 'launch' ?
        ...
        (argv[1] || SETTINGS.platform_default)
    ]},
    {name: 'all', alias: 'a', type: String}
  ];
  if (action === 'test') {
    optionDefinitions = optionDefinitions.concat([
      {name: 'reporter', alias: 'r', type: String, defaultValue: SETTINGS.test_reporter},
      {name: 'port', alias: 'P', type: String, defaultValue: SETTINGS.test_port}
    ]);
  } else {
    optionDefinitions.push(
      {name: 'url', alias: 'u', type: String, defaultValue: url || argv[1]}
    );
  }
  ...
  optionDefinitions = optionDefinitions.concat([
    ...
    {name: 'indent', alias: 'i', type: Number, defaultValue:
      ('indent' in defaults ? defaults.indent : 0)
    }
  ]);
